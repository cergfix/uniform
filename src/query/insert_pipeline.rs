...
/// Returns (success, error_string, optional_select_result, fb_id).
pub fn insert(
    table_name: &str,
    row: &mut OwnedRow,
    metadata: &OwnedRow,
    ...
    if row.columns.is_empty() {
        ...
        return (false, "Cannot write empty record".into(), None, String::new());
    }

    if table_name.is_empty() {
        return (false, "Empty tableName specified.".into(), None, String::new());
    }

    // Generate Fb_created if not set
    if !row.columns.contains_key("Fb_created")
        || row.columns.get("Fb_created") == Some(&Value::String(String::new()))
    {
    ...
    // Generate Fb_id if not set
    if !row.columns.contains_key("Fb_id")
        || row.columns.get("Fb_id") == Some(&Value::String(String::new()))
    {
        row.columns.insert(
            "Fb_id".into(),
    ...
    for proc in procs.iter() {
        if proc.enabled && proc.src == table_name && proc.proc_type == ProcType::DeflectWrite {
            if is_proc_allowed(proc, metadata, row) {
    ...
    if !no_buffer && !moved && table.buffer_max_size > 0 {
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::BufferCatch
            {
                if is_proc_allowed(proc, metadata, row) {
                    filter_row_keys(row, &proc.case_query_string);
                    apply_patch(row, proc);
    ...
    if !moved {
        ...
        for proc in procs.iter() {
            if proc.enabled && proc.src == table_name && proc.proc_type == ProcType::MirrorWrite {
                if is_proc_allowed(proc, metadata, row) {
                    if registry::get_table(&proc.dest).is_some() {
                        ...
                        let (ok, err, _, _) =
                            insert(&proc.dest, &mut nr, metadata, false, _conn);
                        if !ok && !err.is_empty() {
                            if logging::get_log_level() >= logging::LOG_LEVEL_CRIT && !table.silent
                            {
                                logging::log(&format!(
        ...
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::GunzipWrite
            {
                if is_proc_allowed(proc, metadata, row) {
                    filter_row_keys(row, &proc.case_query_string);
                    apply_patch(row, proc);
        ...
        // ========== STAGE 6: GZIP_WRITE ==========
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::GzipWrite {
                if is_proc_allowed(proc, metadata, row) {
                    filter_row_keys(row, &proc.case_query_string);
                    apply_patch(row, proc);
        ...
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::DecryptWrite
            {
                if is_proc_allowed(proc, metadata, row) {
                    filter_row_keys(row, &proc.case_query_string);
                    apply_patch(row, proc);
        ...
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::EncryptWrite
            {
                if is_proc_allowed(proc, metadata, row) {
                    filter_row_keys(row, &proc.case_query_string);
                    apply_patch(row, proc);
        ...
        for proc in procs.iter() {
            if proc.enabled && proc.src == table_name && proc.proc_type == ProcType::AutoReply {
                if is_proc_allowed(proc, metadata, row) {
                    if registry::get_table(&proc.dest).is_some() {
                        ...
                        reply
                            .columns
                            .insert("Fb_body".into(), Value::String(proc.reply_body.clone()));
                        if let Some(id) = row.columns.get("Fb_id") {
                            reply.columns.insert("Fb_reply_id".into(), id.clone());
                        }
                        filter_row_keys(&mut reply, &proc.case_query_string);
                        apply_patch(&mut reply, proc);

                        let (ok, err, _, _) =
                            insert(&proc.dest, &mut reply, metadata, false, _conn);
                        if !ok && !err.is_empty() {
                            if logging::get_log_level() >= logging::LOG_LEVEL_CRIT && !table.silent
                            {
                                logging::log(&format!(
            ...
            let mut long_poll_success = false;
            {
                ...
                if !clients.is_empty() {
                    for client in clients.iter_mut() {
                        if !client.is_expired() && client.is_available() {
                            if client.try_deliver(row.clone()) {
                                if !read_only_mode {
                                    ...
                                    && proc.proc_type == ProcType::DeflectWriteOnFullTable
                                {
                                    if is_proc_allowed(proc, metadata, row) {
                                        if registry::get_table(&proc.dest).is_some() {
                                            ...
                                            let (ok, err, _, _) = insert(
                                                &proc.dest, row, metadata, false, _conn,
                                            );
                                            if !ok && !err.is_empty() {
                                                if logging::get_log_level()
                                                    >= logging::LOG_LEVEL_CRIT
                                                    && !table.silent
                                                {
                                                    logging::log(&format!(
                                                ...
                                                return (true, String::new(), None, fb_id);
                                            }
                                        }
                                        post_wait(proc);
                                        break;
    ...
    (true, String::new(), insert_select_result, fb_id)
}

// ---------- Helper functions ----------

/// Check if a proc is allowed to run (based on case_query_string conditions).
fn is_proc_allowed(proc: &Proc, _metadata: &OwnedRow, _row: &OwnedRow) -> bool {
    if proc.case_query_string.is_empty() {
        return true;
    }
    // TODO: evaluate case_query_string conditions against metadata/row
    true
}
...
fn find_deflect_read(
    ...
    let procs = PROCS.read();
    for proc in procs.iter() {
        if proc.enabled && proc.src == table_name && proc.proc_type == ProcType::DeflectRead {
            if is_proc_allowed(proc, _metadata, &OwnedRow::new()) {
                if proc.post_wait_ms > 0 {
                    std::thread::sleep(std::time::Duration::from_millis(proc.post_wait_ms as u64));
                }
...
pub fn check_read_offline(table_name: &str, metadata: &OwnedRow) -> bool {
    let procs = PROCS.read();
    for proc in procs.iter() {
        if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::ReadOffline {
            if is_proc_allowed(proc, metadata, &OwnedRow::new()) {
                return true;
            }
        }
    }
...
pub fn check_read_only(table_name: &str, metadata: &OwnedRow) -> bool {
    let procs = PROCS.read();
    for proc in procs.iter() {
        if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::ReadOnly {
            if is_proc_allowed(proc, metadata, &OwnedRow::new()) {
                return true;
            }
        }
...
pub fn run_select_pipeline(
    ...
    for patch_opt in deflect_patches {
    ...
    if rows.len() > 1 {
        for proc in procs.iter() {
            if proc.enabled && proc.table == table_name && proc.proc_type == ProcType::ReadReduce {
                if is_proc_allowed(proc, metadata, &OwnedRow::new()) {
    ...
    if rows.is_empty() {
        ...
        if let Some((new_query, patch_data, post_wait_ms)) = deflect_info {
            ...
            let result = crate::query::engine::run_query(
                &new_query,
                None,
                &mut local_conn.metadata,
                None,
            );
    ...
    if !rows.is_empty() {
        for proc in procs.iter() {
            if proc.enabled && proc.src == table_name && proc.proc_type == ProcType::MirrorOnRead {
                if is_proc_allowed(proc, metadata, &OwnedRow::new()) {
                    if registry::get_table(&proc.dest).is_some() {
                        for row in rows.iter() {
